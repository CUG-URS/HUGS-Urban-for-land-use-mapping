...
% hist_spec2='E:\experiment\shenzhen\slic\patch_spe_sift.mat';
% hist1='E:\experiment\shenzhen\slic\patch_spe_sift.mat';
% hist1 = load(hist1);
% hist1=cell2mat(struct2cell(hist1));
% hist2='E:\experiment\shenzhen\slic\poi_98_new.mat';
% hist2 = load(hist2);
% hist2=cell2mat(struct2cell(hist2));
...
% %     feature=[feature;hist1(index0,3:end),hist2(i,:),hist2(i,:),hist2(i,:),hist2(i,:),hist2(i,:),hist2(i,:),hist2(i,:)];
%     feature=[feature;hist1(index0,3:end)];
%     y=[y;hist1(index0,1)];
% end

hist='E:\experiment\shenzhen\slic\NEW\feafinal_ppoi_150.mat';
hist = load(hist);
...
id=[];
for i=1:6910
    if(sum(hist(i,3:386))~=0)
        feature_part=hist(i,3:386);
        feature=[feature;feature_part];
        label=hist(i,2);
        y=[y;label];
        id_part=hist(i,1);
        id=[id;id_part];
...
%                 path='E:\experiment\shenzhen\fstm\spe_sift_ocr-0.3';
...
%            for x=1:row
%                index=A(x,2);
%                 if(sz_patch(index)<=sz_patch(6500))
%                     weakall=weakall+1;
%                    if(A(x,1)==A(x,3))
%                       weak=weak+1;
%                    end
